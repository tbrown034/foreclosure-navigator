...
### The live AI features

The page exposes two: the upload-door read (`api/extract.ts` PDF mode) and the AI summary (`api/summary.ts`). Two more endpoints remain deployed but unexposed (sample-text extraction, sentence polish — their tested exhibits stand on /methodology). All are optional, clearly marked, disclose exactly what is transmitted (and that Anthropic's standard API retention applies — the demo itself stores nothing), and degrade gracefully — the page keeps working when any endpoint is absent or over quota. Rate limits (extraction: 6/IP/hour, 100/day; summary: 6/IP/hour, 150/day; polish: 5/IP/hour, 200/day) fail closed; counters are in-memory per serverless instance, so approximate deployment-wide but strict per instance — production would use a shared store. Upstream model calls carry a hard timeout. There are no other AI endpoints.
...
- **`api/summary.ts` — the AI summary.** The one generative moment on the reader's path. The server recomputes the stage assessment in code from the submitted dates (never trusting client text), Claude Haiku restates those facts as 3-5 plain second-person sentences, and the polish token guard — extended to license full month names whose abbreviations appear in the facts — rejects any output containing a date, digit run or number word not in the computed facts. A rejection tells the reader the computed panel stands.
