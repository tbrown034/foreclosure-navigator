...
export function guardFacts(facts: string): string {
  return facts + " " + MONTH_EXPANSIONS.filter(([re]) => re.test(facts)).map(([, full]) => full).join(" ");
}
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const invented = inventedTokens(guardFacts(facts), summary);
